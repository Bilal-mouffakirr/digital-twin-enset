...
from plotly.subplots import make_subplots   # ✅ Fix #1 — import hors boucle
...
def run_pvlib_simulation(lat, lon, altitude, timezone, tilt, azimuth, pdc0, gamma_pdc, start_date, end_date):
    ...
    cell_temp = temperature.sapm_cell(
        poa_global=poa_iso["poa_global"], temp_air=df["temp_air"],
    ...
    net = ac * (1 - 0.10)
    res = pd.DataFrame({
        "datetime": times, "ghi": df["ghi"], "dni": df["dni"], "dhi": df["dhi"],
        "poa_isotropic": poa_iso["poa_global"], "poa_haydavies": poa_hd["poa_global"],
        "temp_air": df["temp_air"], "cell_temp": cell_temp,
        "wind_speed": df["wind_speed"], "humidity": df["humidity"],
        "cloud_cover": df["cloud_cover"],
        "dc_power_w": dc, "ac_power_w": ac.clip(lower=0), "net_ac_power_w": net.clip(lower=0),
        "solar_elevation": solar_pos["elevation"], "solar_azimuth": solar_pos["azimuth"],
        "dni_extra": dni_extra
    })
    res["ac_power_kw"] = res["ac_power_w"] / 1000
    res["net_ac_power_kw"] = res["net_ac_power_w"] / 1000
    res["date"] = res["datetime"].dt.date
    res["hour"] = res["datetime"].dt.hour
    res["month"] = res["datetime"].dt.month
    res["month_name"] = res["datetime"].dt.strftime("%b")
...
def compute_daily(results):
    ...
    daily = results.groupby("date").agg(
        ...
        avg_temp=("temp_air", "mean"), max_temp=("temp_air", "max"), min_temp=("temp_air", "min"),
        avg_ghi=("ghi", "mean"), max_ghi=("ghi", "max"),
        avg_wind=("wind_speed", "mean"), avg_humidity=("humidity", "mean"),
        avg_cloud=("cloud_cover", "mean"),
        peak_sun_hours=("poa_isotropic", lambda x: x.sum() / 1000),
        theoretical_psh=("ghi", lambda x: x.sum() / 1000)
    ).reset_index()
    # ✅ Fix #2 — clip pour éviter theoretical_kwh négatif
    daily["theoretical_kwh"] = (daily["peak_sun_hours"] * cap).clip(lower=0)
...
def calculate_degraded_power(pdc0, years, rate):
    return pdc0 * (1 - rate) ** years


def estimate_co2_avoidance(kwh, ef=0.65):
    return kwh * ef


def calculate_financial_metrics(kwh, price=0.12):
    return kwh * price
...
# ════════════════════════════════════════════════════════
@st.cache_data(ttl=60)   # ✅ Fix #3 — ttl réduit à 60s pour éviter de cacher les erreurs longtemps
def fetch_meteo_now():
    now = datetime.now()
...
def fmu_step(G, Tamb, wind):
    ...
    Pac = Pb * ei
    cp = 0.99
    # ✅ Fix #4 — THD non physique à charge nulle
    if Ppv > 10:
        THDv = 1.8 + 0.5 * (1 - rat)
        THDi = 3.0 + 2.0 * (1 - rat)
    else:
        ...
        THDi = 0.0
    return dict(
        Tc=Tc, Vmpp=Vmpp, Impp=Impp, Ppv=Ppv, Pboost=Pb,
...
def pvlib_step_rt(GHI, DHI, DNI, Tamb, wind, ts):
    try:
        ...
        tp = temperature.TEMPERATURE_MODEL_PARAMETERS["sapm"]["open_rack_glass_glass"]
        Tc = temperature.sapm_cell(
            pd.Series([pg], index=t), pd.Series([Tamb], index=t),
...
with st.sidebar:
    ...
    # ✅ Fix #5 — Lire session_state AVANT de créer les widgets date
    default_start = st.session_state.pop("_ps", datetime(2026, 4, 1).date())
    default_end   = st.session_state.pop("_pe", datetime.today().date())

    col_s, col_e = st.columns(2)
    with col_s:
        st.caption("Début")
        start_date = st.date_input("Debut", value=default_start, label_visibility="collapsed")
    with col_e:
        st.caption("Fin")
        end_date = st.date_input("Fin", value=default_end, label_visibility="collapsed")

    preset_cols = st.columns(3)
    with preset_cols[0]:
        if st.button("7j", key="7d"):
            st.session_state["_ps"] = (datetime.now() - timedelta(days=7)).date()
            st.session_state["_pe"] = datetime.now().date()
            st.rerun()
    with preset_cols[1]:
    ...
    # ✅ Fix #6 — data_age ne peut pas être négatif
    data_age = max((datetime.now().date() - end_date).days, 0)
    if data_age == 0:
        st.markdown('<div class="data-info">Données actualisées aujourd\'hui</div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="data-info">Actualisées il y a {data_age} jour{"s" if data_age > 1 else ""}</div>', unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown(f"**Dégradation** après {years_op} ans : **{deg:.0f} W** (-{(1 - deg / PANEL['pdc0']) * 100:.1f}%)")
...
elif ghi_now > 50:  sl, sc = "PRODUCTION RÉDUITE",   "var(--solar-yellow)"
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px">
    ...
    <div style="display:flex;align-items:center;gap:16px">
      <div style="text-align:right">
        ...
        <div style="font-size:10px;color:#3D4553;font-family:'Space Mono',monospace">UTC+1 &nbsp;|&nbsp; GHI: {ghi_now:.0f} W/m² &nbsp;|&nbsp; Cloud: {cloud_now}%</div>
      </div>
...
if menu == "⚡ FMU Temps Réel":
    ...
    with ct1:
        if st.button("▶ START" if not st.session_state.fmu_run else "⏸ PAUSE", type="primary", use_container_width=True):
            st.session_state.fmu_run = not st.session_state.fmu_run
            st.rerun()
    with ct2:
        if st.button("↺ Reset", use_container_width=True):
            for k in FMU_KEYS:
                st.session_state[k] = []
            st.session_state.fmu_tick = 0
            st.session_state.fmu_wx = None
    with ct3:
        spd = st.select_slider("", options=[1, 2, 3, 5], value=1,
                               label_visibility="collapsed",
    ...
    r1 = st.columns(5)
    ph_GHI = r1[0].empty(); ph_DNI = r1[1].empty(); ph_DHI = r1[2].empty()
    ph_Tamb = r1[3].empty(); ph_Tc = r1[4].empty()
    ...
    def fkcard(val, unit, lbl, src, ac):
        return (f'<div class="fmu-kcard" style="--ac:{ac}">'
                f'<div class="fmu-kval">{val}<span class="fmu-kunit">{unit}</span></div>'
                f'<div class="fmu-klbl">{lbl}</div>'
                f'<div class="fmu-ksrc">{src}</div></div>')

    def mfig(h=265):
        f = go.Figure()
        f.update_layout(height=h, **FMU_BG, margin=FMU_MAR, legend=FMU_LEG, font=FMU_FNT,
                        xaxis=dict(**FMU_GRD, tickformat="%H:%M:%S"),
                        yaxis=dict(**FMU_GRD))
        return f
...
<div class="flow-bar">
  ...
  <span style="color:#4b6080;font-size:.68rem">tick</span><span class="flow-chip">#{h.fmu_tick:05d}</span>
            ...
            ph_Vac.markdown( fkcard("220",               "V",    "Vonduleur","FMU output",              "#22c55e"), unsafe_allow_html=True)
            ...
            dlt = fv["Ppv"] - Ppl
            dc_col = "#22c55e" if abs(dlt) < 100 else "#f59e0b" if abs(dlt) < 300 else "#ef4444"
            ...
            f2.update_layout(
                yaxis=dict(title="GHI [W/m²]", **FMU_GRD),
                yaxis2=dict(title="Temp [°C]", overlaying="y", side="right",
                            showgrid=False, color="#3b82f6", tickfont=dict(size=9)),
            ...
            f3.update_xaxes(**FMU_GRD, tickformat="%H:%M:%S")
            f3.update_yaxes(**FMU_GRD)
...
# ════════════════════════════════════════════════════════
    ...
    co2 = estimate_co2_avoidance(total_kwh)
    savings = calculate_financial_metrics(total_kwh)
    ...
    kpis = [
        ...
        (f"{co2:.0f}",            "CO2 Évité<br>kg"),
        (f"{savings:.0f}",        "Économies<br>EUR"),
        (f"{avg_cf:.1f}",         "Facteur<br>Capacité %"),
    ]
    ...
    with col_img:
        wc = st.columns(4)
        wc[0].metric("Température",  f"{current_meteo.get('temperature_2m','--')} °C")
        wc[1].metric("Irradiance",   f"{ghi_now:.0f} W/m²")
        wc[2].metric("Vent",         f"{current_meteo.get('wind_speed_10m','--')} km/h")
        wc[3].metric("Humidité",     f"{current_meteo.get('relative_humidity_2m','--')} %")
    with col_perf:
    ...
    with col_left:
        ...
        fig_d.add_trace(go.Scatter(x=daily["date"], y=daily["production_kwh"].rolling(7, center=True).mean(),
                                   name="Moy. 7j", line=dict(color="#3498DB", width=2)))
        l = dict(**PLOT_LAYOUT)
        l["height"] = 320; l["showlegend"] = True
        l["yaxis"] = dict(gridcolor="#1E232D", title="kWh", zeroline=False)
        fig_d.update_layout(**l)
    ...
    with col_right:
        ...
        dw["day_of_week"] = pd.Categorical(dw["day_of_week"],
        ...
        l2 = dict(**PLOT_LAYOUT); l2["height"] = 320
        fig_w.update_layout(**l2)
    ...
    with ac1:
        lpd = daily[daily["pr"] < 70]
        htd = daily[daily["avg_temp"] > 35]
        if len(lpd) > 0:
            st.markdown(f'<div class="alert-card alert-warning"><b>Performance dégradée</b><br>{len(lpd)} jours PR&lt;70% — Dernier: {lpd["date"].max().strftime("%d/%m/%Y")}</div>', unsafe_allow_html=True)
        if len(htd) > 0:
            st.markdown(f'<div class="alert-card alert-warning"><b>Température élevée</b><br>{len(htd)} jours &gt;35°C</div>', unsafe_allow_html=True)
        st.markdown('<div class="alert-card alert-ok"><b>Système de monitoring</b> — Toutes les données reçues</div>', unsafe_allow_html=True)
    with ac2:
        hs = max(min(100 - len(lpd) * 5 - len(htd) * 2, 100), 0)
        hc = "#2ECC71" if hs > 80 else "#F5A623" if hs > 60 else "#E74C3C"
        st.markdown(f'<div class="kpi-card"><div class="kpi-value" style="color:{hc}">{hs:.0f}/100</div><div class="kpi-label">Score de<br>Santé Système</div></div>', unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════
    ...
    with t1:
        ...
        with c1:
            ...
            fp = go.Figure()
            fp.add_trace(go.Scatter(x=daily["date"], y=daily["pr"], fill="tozeroy",
                                    fillcolor="rgba(245,166,35,0.10)",
                                    line=dict(color="#F5A623", width=1.5), name="PR %"))
            fp.add_hline(y=75, line_dash="dash", line_color="#3D4553",
                         annotation_text="75%", annotation_font_color="#6B7585", annotation_font_size=11)
            fp.add_hline(y=85, line_dash="dash", line_color="#2ECC71",
                         annotation_text="85%", annotation_font_color="#2ECC71", annotation_font_size=11)
            lp = dict(**PLOT_LAYOUT); lp["height"] = 350
            lp["yaxis"] = dict(gridcolor="#1E232D", range=[0, 110], zeroline=False)
            fp.update_layout(**lp)
            st.plotly_chart(fp, use_container_width=True)
        with c2:
            st.markdown('<div class="section-title">Facteur de capacité (%)</div>', unsafe_allow_html=True)
            fc = go.Figure()
            fc.add_trace(go.Scatter(x=daily["date"], y=daily["capacity_factor"], fill="tozeroy",
                                    fillcolor="rgba(52,152,219,0.10)",
                                    line=dict(color="#3498DB", width=1.5), name="CF %"))
            lc = dict(**PLOT_LAYOUT); lc["height"] = 350
            lc["yaxis"] = dict(gridcolor="#1E232D", title="%", zeroline=False)
            fc.update_layout(**lc)
            st.plotly_chart(fc, use_container_width=True)

        st.markdown('<div class="section-title">Corrélations</div>', unsafe_allow_html=True)
        cc = st.columns(3)
        for ax, ay, t_c in [("avg_temp","production_kwh","Temp vs Prod"),
                             ("peak_sun_hours","production_kwh","PSH vs Prod"),
                             ("avg_wind","production_kwh","Vent vs Prod")]:
            with cc.pop(0):
                fc2 = px.scatter(daily, x=ax, y=ay, color="pr",
                                 color_continuous_scale="YlOrRd", opacity=0.7, title=t_c)
                fc2.update_layout(**{**PLOT_LAYOUT, "height": 300})
    ...
    with t2:
        ...
        fh = go.Figure()
        fh.add_trace(go.Scatter(x=hs["hour"], y=hs["max"], fill=None, mode="lines",
                                line=dict(color="rgba(245,166,35,0.2)", width=0), showlegend=False))
        fh.add_trace(go.Scatter(x=hs["hour"], y=hs["min"], fill="tonexty",
                                fillcolor="rgba(245,166,35,0.1)", mode="lines",
                                line=dict(color="rgba(245,166,35,0.2)", width=0), name="Plage"))
        fh.add_trace(go.Scatter(x=hs["hour"], y=hs["mean"],
                                line=dict(color="#F5A623", width=2.5), name="Moyenne"))
        lh = dict(**PLOT_LAYOUT); lh["height"] = 350
        lh["xaxis"] = dict(title="Heure", gridcolor="#1E232D", dtick=1, zeroline=False)
        lh["yaxis"] = dict(title="kW", gridcolor="#1E232D", zeroline=False)
        fh.update_layout(**lh)
        st.plotly_chart(fh, use_container_width=True)

    with t3:
        st.markdown('<div class="section-title">Heatmap Mois x Heure</div>', unsafe_allow_html=True)
        hp = results.groupby(["month", "hour"])["ac_power_kw"].mean().reset_index().pivot(
            index="month", columns="hour", values="ac_power_kw")
        mfr = ["Jan","Fév","Mar","Avr","Mai","Jun","Jul","Aoû","Sep","Oct","Nov","Déc"]
        hp.index = [mfr[m-1] for m in hp.index]
        fhm = go.Figure(go.Heatmap(
            z=hp.values, x=[f"{h}h" for h in hp.columns], y=hp.index,
            colorscale=[[0,"#08090C"],[0.4,"#2A1500"],[0.8,"#E8860A"],[1,"#F5A623"]],
            showscale=True, colorbar=dict(title="kW", tickfont=dict(color="#6B7585"))
        ))
        lhm = dict(**PLOT_LAYOUT); lhm["height"] = 400
        fhm.update_layout(**lhm)
...
# ════════════════════════════════════════════════════════
    ...
    for i, (lbl, key, unit) in enumerate([
        ("Température","temperature_2m","°C"),("Ressentie","apparent_temperature","°C"),
        ("Humidité","relative_humidity_2m","%"),("Vent","wind_speed_10m","km/h"),
        ("GHI","shortwave_radiation","W/m²"),("Nuages","cloud_cover","%")
    ]):
        with wk[i]:
            st.metric(lbl, f"{current_meteo.get(key,'--')} {unit}")
    st.markdown("---")

    it = st.tabs(["GHI & POA", "Température", "Ressource Solaire"])
    with it[0]:
        cl, cr = st.columns(2)
        with cl:
            st.markdown('<div class="section-title">GHI journalier</div>', unsafe_allow_html=True)
            dg = results.groupby("date").agg(ghi_sum=("ghi", lambda x: x.sum() / 1000)).reset_index()
            dg["date"] = pd.to_datetime(dg["date"])
            fg = go.Figure()
            fg.add_trace(go.Scatter(x=dg["date"], y=dg["ghi_sum"], fill="tozeroy",
                                    fillcolor="rgba(245,166,35,0.12)",
                                    line=dict(color="#F5A623", width=1.5), name="GHI"))
            lg = dict(**PLOT_LAYOUT); lg["height"] = 350
            lg["yaxis"] = dict(gridcolor="#1E232D", title="kWh/m²", zeroline=False)
            fg.update_layout(**lg)
            st.plotly_chart(fg, use_container_width=True)
        with cr:
            st.markdown('<div class="section-title">POA Irradiance</div>', unsafe_allow_html=True)
            dp = results.groupby("date").agg(
                poa_iso=("poa_isotropic", "mean"), poa_hd=("poa_haydavies", "mean")).reset_index()
            dp["date"] = pd.to_datetime(dp["date"])
            fp2 = go.Figure()
            fp2.add_trace(go.Scatter(x=dp["date"], y=dp["poa_iso"],
                                     line=dict(color="#F5A623", width=1.5), name="Isotropique"))
            fp2.add_trace(go.Scatter(x=dp["date"], y=dp["poa_hd"],
                                     line=dict(color="#3498DB", width=1.5), name="Hay-Davies"))
            lp2 = dict(**PLOT_LAYOUT); lp2["height"] = 350
            lp2["yaxis"] = dict(gridcolor="#1E232D", title="W/m²", zeroline=False)
            fp2.update_layout(**lp2)
            st.plotly_chart(fp2, use_container_width=True)

    with it[1]:
        ct1, ct2 = st.columns(2)
        with ct1:
            st.markdown('<div class="section-title">Température ambiante</div>', unsafe_allow_html=True)
            dt = results.groupby("date").agg(
                ta=("temp_air","mean"), tmx=("temp_air","max"), tmn=("temp_air","min")).reset_index()
            dt["date"] = pd.to_datetime(dt["date"])
            ft = go.Figure()
            ft.add_trace(go.Scatter(x=dt["date"], y=dt["tmx"], line=dict(color="#E74C3C", width=1), name="Max"))
            ...
            lt = dict(**PLOT_LAYOUT); lt["height"] = 350
            lt["yaxis"] = dict(gridcolor="#1E232D", title="°C", zeroline=False)
            ft.update_layout(**lt)
        ...
        with ct2:
            ...
            fc3 = go.Figure()
            fc3.add_trace(go.Scatter(x=ht["hour"], y=ht["temp_air"],  line=dict(color="#3498DB", width=2), name="Ambiante"))
            fc3.add_trace(go.Scatter(x=ht["hour"], y=ht["cell_temp"], line=dict(color="#E74C3C", width=2), name="Cellule"))
            lc3 = dict(**PLOT_LAYOUT); lc3["height"] = 350
            lc3["xaxis"] = dict(title="Heure", gridcolor="#1E232D", dtick=1)
            lc3["yaxis"] = dict(title="°C", gridcolor="#1E232D")
            fc3.update_layout(**lc3)
    ...
    with it[2]:
        ...
        mfr2 = ["Jan","Fév","Mar","Avr","Mai","Jun","Jul","Aoû","Sep","Oct","Nov","Déc"]
        ms = results.groupby("month").agg(
            g=("ghi", lambda x: x.sum() / 1000),
            d=("dni", lambda x: x.sum() / 1000),
            h=("dhi", lambda x: x.sum() / 1000)).reset_index()
        ms["mn"] = [mfr2[m-1] for m in ms["month"]]
        fs = go.Figure()
        fs.add_trace(go.Bar(x=ms["mn"], y=ms["g"], name="GHI", marker_color="#F5A623"))
        ...
        ls = dict(**PLOT_LAYOUT); ls["height"] = 300; ls["barmode"] = "group"
        ls["yaxis"] = dict(gridcolor="#1E232D", title="kWh/m²")
        fs.update_layout(**ls)
...
# ════════════════════════════════════════════════════════
    ...
    pk[3].metric("Disponibilité",  "98.5%")
    st.markdown("---")
    ...
    with cp1:
        ...
        fd = go.Figure()
        fd.add_trace(go.Histogram(x=daily["pr"].dropna(), nbinsx=30, marker_color="#F5A623"))
        ld = dict(**PLOT_LAYOUT); ld["height"] = 300
        ld["xaxis"] = dict(title="PR (%)", gridcolor="#1E232D")
        ld["yaxis"] = dict(title="Fréq", gridcolor="#1E232D")
        fd.update_layout(**ld)
        st.plotly_chart(fd, use_container_width=True)
    with cp2:
        st.markdown('<div class="section-title">PR Mensuel</div>', unsafe_allow_html=True)
        fm = go.Figure()
        fm.add_trace(go.Bar(x=monthly["month_name"], y=monthly["avg_pr"], marker_color="#F5A623"))
        lm = dict(**PLOT_LAYOUT); lm["height"] = 300
        lm["yaxis"] = dict(gridcolor="#1E232D", title="PR %", range=[50, 100])
        fm.update_layout(**lm)
        st.plotly_chart(fm, use_container_width=True)

    ce1, ce2 = st.columns(2)
    with ce1:
        fey = go.Figure(go.Scatter(x=daily["date"], y=daily["energy_yield"], fill="tozeroy",
                                   fillcolor="rgba(26,188,156,0.1)",
                                   line=dict(color="#1ABC9C", width=1.5)))
        ley = dict(**PLOT_LAYOUT); ley["height"] = 300
        ley["yaxis"] = dict(gridcolor="#1E232D", title="kWh/kWp")
        fey.update_layout(**ley)
        st.plotly_chart(fey, use_container_width=True)
    with ce2:
        daily["cum_kwh"] = daily["production_kwh"].cumsum()
        fcu = go.Figure(go.Scatter(x=daily["date"], y=daily["cum_kwh"] / 1000,
                                   line=dict(color="#9B59B6", width=2),
                                   fill="tozeroy", fillcolor="rgba(155,89,182,0.1)"))
        lcu = dict(**PLOT_LAYOUT); lcu["height"] = 300
        lcu["yaxis"] = dict(gridcolor="#1E232D", title="MWh")
        fcu.update_layout(**lcu)
        st.plotly_chart(fcu, use_container_width=True)
...
# ════════════════════════════════════════════════════════
    ...
    for i in range(n):
        with ic[i % min(n, 6)]:
            ...
            st.markdown(
                f'<div style="background:#111318;border:1px solid #252A35;border-radius:8px;'
                f'padding:12px 10px;text-align:center;border-left:3px solid {col}">'
                f'<div style="font-size:10px;color:#6B7585;font-family:Space Mono,monospace">{il[i]}</div>'
                f'<div style="font-size:20px;font-weight:700;color:{col};font-family:Space Mono,monospace">{ip[i]:.0f}%</div>'
                f'<div style="font-size:10px;color:{col}">{ist[i]}</div></div>',
                unsafe_allow_html=True
            )
    st.markdown("---")
    oc1, oc2 = st.columns(2)
    with oc1:
        fib = go.Figure(go.Bar(x=il, y=ip,
                               marker_color=["#F5A623" if p < 85 else "#2ECC71" for p in ip]))
        fib.add_hline(y=85, line_dash="dash", line_color="#E74C3C", annotation_text="85%")
        lib = dict(**PLOT_LAYOUT); lib["height"] = 300
        fib.update_layout(**lib)
        st.plotly_chart(fib, use_container_width=True)
    with oc2:
        ok = sum(1 for p in ip if p >= 85)
        fip = go.Figure(go.Pie(
            labels=["Nominaux", "Alertes"], values=[ok, n - ok], hole=0.6,
            marker_colors=["#2ECC71", "#F5A623"]
        ))
        fip.add_annotation(text=f"<b>{n}</b><br>Total", x=0.5, y=0.5,
                           showarrow=False, font=dict(size=14, color="#E8EDF5"))
        fip.update_layout(template="plotly_dark", paper_bgcolor="#111318",
...
# ════════════════════════════════════════════════════════
    ...
    ki1.metric("Puissance crête", f"{SITE['capacity_kwp']} kWp")
...
<div class="spec-block">
  <div class="spec-block-header">Site & Générale</div>
  <table class="spec-table">
    <tr><td>Localisation</td><td>Mohammedia, Maroc</td></tr>
    <tr><td>Latitude</td><td><span class="highlight-value">{SITE["lat"]} N</span></td></tr>
    <tr><td>Longitude</td><td><span class="highlight-value">{abs(SITE["lon"])} W</span></td></tr>
    <tr><td>Altitude</td><td>{SITE["altitude"]} m</td></tr>
    <tr><td>Mise en service</td><td>{SITE["commissioning_date"]}</td></tr>
    ...
    with ib:
        st.markdown(f"""
<div class="spec-block">
  <div class="spec-block-header">Panneau PV</div>
  <table class="spec-table">
    <tr><td>Fabricant</td><td>{PANEL["manufacturer"]}</td></tr>
    <tr><td>Modèle</td><td>{PANEL["model"]}</td></tr>
    <tr><td>Pmax</td><td><span class="highlight-value">{PANEL["pdc0"]} Wc</span></td></tr>
...
# ════════════════════════════════════════════════════════
    ...
    st.markdown("## Contrôle Relais ESP32 via Blynk")
    dc = blynk_get_device_status()
    if dc:
        st.success("ESP32 connecté à Blynk")
    else:
        st.warning("ESP32 non connecté")

    rs = blynk_get_pin(BLYNK_CONFIG["relay_pin"])
    # ✅ Fix #8 — correction du type pour comparer l'état du relais
    relay_raw   = rs[0] if isinstance(rs, list) else rs
    is_active   = str(relay_raw) == "1"

    rc1, rc2, rc3 = st.columns(3)
    with rc1:
        col = "#2ECC71" if is_active else "#E74C3C"
        st.markdown(
            f'<div class="metric-card"><div class="metric-label">ÉTAT RELAIS</div>'
            f'<div class="metric-value" style="color:{col}">{"ACTIF" if is_active else "INACTIF"}</div></div>',
            unsafe_allow_html=True
        )
    with rc2:
        st.markdown(
            f'<div class="metric-card"><div class="metric-label">PIN BLYNK</div>'
            f'<div class="metric-value">{BLYNK_CONFIG["relay_pin"]}</div></div>',
            unsafe_allow_html=True
        )
    with rc3:
        col2 = "#2ECC71" if dc else "#E74C3C"
        # ✅ Fix #9 — "FAR" remplacé par "EN LIGNE"
        st.markdown(
            f'<div class="metric-card"><div class="metric-label">CONNEXION</div>'
            f'<div class="metric-value" style="color:{col2}">{"EN LIGNE" if dc else "HORS LIGNE"}</div></div>',
            unsafe_allow_html=True
        )
    st.markdown("---")
    ...
    with rb1:
        if st.button("ACTIVER RELAIS", type="primary", use_container_width=True):
            if blynk_set_pin(BLYNK_CONFIG["relay_pin"], 1):
                st.success("Relais activé")
                st.rerun()
            else:
                st.error("Erreur ESP32")
    with rb2:
        if st.button("DÉSACTIVER RELAIS", type="secondary", use_container_width=True):
            if blynk_set_pin(BLYNK_CONFIG["relay_pin"], 0):
                st.success("Relais désactivé")
                st.rerun()
            else:
                st.error("Erreur ESP32")
    with rb3:
...
# ════════════════════════════════════════════════════════
    ...
    gross_kwh = daily["gross_production_kwh"].sum()
    avg_pr    = daily["pr"].mean()
    bm = monthly.loc[monthly["production_kwh"].idxmax()]
    wm = monthly.loc[monthly["production_kwh"].idxmin()]
...
border-radius:12px;padding:24px 28px 16px;margin-bottom:20px;">
  <div style="font-family:'Space Mono',monospace;font-size:15px;color:#F5A623;font-weight:700;
  text-transform:uppercase;letter-spacing:0.1em;margin-bottom:6px;">Rapport de Performance</div>
  <div style="font-size:12px;color:#6B7585;font-family:'Space Mono',monospace;">
    ...
    st.markdown(f"""
<div style="{S_CARD}"><div style="{S_HEAD}">Production énergétique</div>
<table style="{S_TABLE}">
  <tr><td style="{S_TL}">Production totale</td>
      <td style="{S_TR}"><span style="color:#F5A623;font-family:'Space Mono',monospace;font-weight:700;">{total_kwh/1000:.2f} MWh</span></td></tr>
  <tr><td style="{S_TL}">Production journalière moyenne</td>
      <td style="{S_TR}">{daily["production_kwh"].mean():.1f} kWh / jour</td></tr>
  <tr><td style="{S_TL}">Meilleur mois</td>
      <td style="{S_TR}"><span style="color:#2ECC71;font-weight:600;">{bl}</span></td></tr>
  <tr><td style="{S_TLL}">Mois le plus faible</td>
      <td style="{S_TRL}"><span style="color:#E74C3C;font-weight:600;">{wl}</span></td></tr>
</table></div>""", unsafe_allow_html=True)

    st.markdown(f"""
<div style="{S_CARD}"><div style="{S_HEAD}">Indicateurs de performance</div>
<table style="{S_TABLE}">
  <tr><td style="{S_TL}">Performance Ratio moyen</td>
      <td style="{S_TR}"><span style="color:{prc};font-family:'Space Mono',monospace;font-weight:700;">{avg_pr:.1f} %</span></td></tr>
  <tr><td style="{S_TL}">Jours PR &lt; 70%</td>
      <td style="{S_TR}"><span style="color:#E74C3C;">{len(daily[daily['pr']<70])} jours</span></td></tr>
  <tr><td style="{S_TLL}">Rendement énergétique net</td>
      <td style="{S_TRL}">{(total_kwh/gross_kwh*100) if gross_kwh>0 else 0:.1f} %</td></tr>
</table></div>""", unsafe_allow_html=True)
    ...
    with cd1:
        st.download_button(
            "Télécharger journalier (CSV)",
            daily.to_csv(index=False, float_format="%.2f"),
            f"solaris_daily_{start_date}_{end_date}.csv", "text/csv"
        )
    with cd2:
        st.download_button(
            "Télécharger mensuel (CSV)",
            monthly.to_csv(index=False, float_format="%.2f"),
            f"solaris_monthly_{start_date}_{end_date}.csv", "text/csv"
        )
...
if menu != "⚡ FMU Temps Réel":
    st.markdown("---")
    st.markdown(
        f'<div style="text-align:center;color:#252A35;font-size:11px;padding:10px 0;'
        f'font-family:Space Mono,monospace;letter-spacing:0.08em">'
        f'SOLARIS DIGITAL TWIN V3.1 &nbsp;|&nbsp; MOHAMMEDIA, MAROC &nbsp;|&nbsp;'
        f' pvlib + Open-Meteo + FMU Real-Time &nbsp;|&nbsp; {now.strftime("%d/%m/%Y %H:%M")}'
        f'</div>',
        unsafe_allow_html=True
    )
